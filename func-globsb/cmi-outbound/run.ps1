...
try {
    ...
    Send-ErrorEmail `
        -Subject "Azure Function Service Bus publish error" `
        -Body "Service Bus publish failed for topicKey '$topicKey':`n`n$($_ | Out-String)" `
        -Context @{ Function = 'cmi-outbound'; TopicKey = $topicKey }
...
"@
    ...
    Send-ErrorEmail `
        -Subject "Azure Function IB rule execution error" `
        -Body "IB rule execution failed for outbound topicKey '$topicKey':`n`n$($_ | Out-String)" `
        -Context @{ Function = 'cmi-outbound'; TopicKey = $topicKey }
}
