...
if (-not $IbHost)  { Write-Log -Level 'ERROR' -Message "Missing env:intapp__ibHost";   Send-ErrorEmail -Subject "Azure Function configuration error" -Body "Missing env:intapp__ibHost" -Context @{ Function = 'cmi-subscribe-cmi-fails'; Subscriber = $script:SubscriberId }; throw "Missing ibHost" }
if (-not $RuleId)  { Write-Log -Level 'ERROR' -Message "Missing env:intapp__ibRuleId"; Send-ErrorEmail -Subject "Azure Function configuration error" -Body "Missing env:intapp__ibRuleId" -Context @{ Function = 'cmi-subscribe-cmi-fails'; Subscriber = $script:SubscriberId }; throw "Missing ibRuleId" }
if (-not $IbToken) { Write-Log -Level 'ERROR' -Message "Missing env:intapp__ibToken";  Send-ErrorEmail -Subject "Azure Function configuration error" -Body "Missing env:intapp__ibToken" -Context @{ Function = 'cmi-subscribe-cmi-fails'; Subscriber = $script:SubscriberId }; throw "Missing ibToken" }
...
try {
    ...
    Send-ErrorEmail `
        ...
        -Body "Unhandled cmi-fails subscriber error:`n`n$($_ | Out-String)" `
        -Context @{ Function = 'cmi-subscribe-cmi-fails'; Subscriber = $script:SubscriberId }
    throw
}
