...
function Get-MessageType {
    ...
    # 1) Prefer an explicit type inside the payload if present
    ...
    # 2) Fall back to topic parsing (works for "compliance.clients.v1" etc.)
    if ($Topic) {
...
try {
    ...
    # ---------------------------------------------
    $topicForType = $TriggerMetadata.Topic
    $msgType = Get-MessageType -Topic $topicForType -ParsedObject $obj
    ...
    try {
        $null = Invoke-RestMethod -Method Put -Uri $ApiUrl `
    ...
    # NOTE:
    # Since this function is now *reading from cmi-fails*, re-publishing to "fails"
    # can create a loop. Let the runtime retry / dead-letter based on host settings.
    throw
}
